...
\chapter{Przegląd metod analiz sieci (też temporalnych) oraz analiz blockchaina}

\section{Wprowadzenie}

\indent Materiał zawarty w tym rozdziale ma służyć jako przegląd istniejących podejść badania sieci. Opisuje historię rozwoju sieci, która ma istotne znaczenie dla podziału sieci ze względu na ich charakterystykę. Przedstawiono możliwą klasyfikację sieci, opartą na zastosowanej w nich teorii oraz topologii. Następnie opisano metody analizy właściwości sieci. W ramach przeglądu istniejących... 
...
\begin{itemize}
\item[--] długość najkrótszej ścieżki w sieci - długość ścieżki $\min_{v_i, v_j} d_{v_iv_j, i \ne j}$, która dla grafu ważonego wymaga przejście od węzła $v_i$ do węzła $v_j$ przez najmniejszą sumarycznie wartość wag $\omega$ przypisanych do krawędzi, gdzie węzły $v_i$ i $v_j$ są węzłami skrajnymi ścieżki, natomiast dla grafu bez wag oznacza długość ścieżki $\min_{v_i, v_j} d_{v_iv_j, i \ne j}$, w~której liczba krawędzi pomiędzy węzłami $v_i$ i $v_j$ jest najmniejsza, a węzły $v_i$ i $v_j$ są węzłami skrajnymi ścieżki;
